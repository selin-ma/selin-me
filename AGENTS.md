...
- **Tailwind over inline styles**: Use Tailwind utility classes for all styling. Never write `style={{}}` inline styles or `React.CSSProperties` type annotations.
- **Exception**: Inline `style` is only allowed for dynamic values that Tailwind cannot express, such as canvas drawing coordinates or JS-computed positions. Font-family is NOT an exception — use the utility classes (`font-pixel-sans`, `font-caveat-hand`, etc.).
- **Colors must use CSS variables**: Always reference colors via the project's design tokens (e.g. `text-ink`, `bg-cream`, `border-sage`). Never write raw hex values like `text-[#1a1a1a]` or `bg-[#c8f135]` inline — if a color is needed, register it in the `@theme` block in `app/globals.css` and use the token.
- Components use TypeScript with `interface` for props types.
- Use Tailwind utility classes; use `cn()` from `lib/utils.ts` for conditional class merging
- Color tokens available in `@theme`: `ink` / `ink-light` / `ink-lt`, `olive` / `olive-lt` / `olive-bg` / `olive-dark`, `amber` / `amber-ink`, `cream` / `cream-lt`, `paper`, `warm-white`, `terra`, `sage`, `dark`, `midnight`, `gold`, `sticky-green`, `sticky-yellow`
- Font families (utility classes): `font-dm-mono` (DM Mono), `font-body` (Inter), `font-pixel-sans` (Pixelify Sans), `font-caveat-hand` (Caveat). Names are aliased to avoid conflicting with next/font's CSS variables (`--font-pixel`, `--font-caveat`, `--font-dm-mono`). Use these classes, NOT `style={{ fontFamily: 'var(--font-*)' }}`.
